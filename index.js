...
// PUT /orders/:id - update order and auto-credit motoboy on delivery
app.put('/orders/:id', async (req, res) => {
  try {
    ...
    // Auto-credit motoboy when status becomes 'entregue'
    if (fields.status === 'entregue' && order.motoboy_id) {
      const valorMotoboy = parseFloat(order.valor_motoboy) || 0;
      if (valorMotoboy > 0) {
        await pool.query(
          'UPDATE users SET balance = balance + $1 WHERE id=$2',
          [valorMotoboy, order.motoboy_id]
        );
      }
      // If cobrar_cliente=sim and tipo_pagamento=dinheiro, credit the loja with valor_pedido
      if (order.cobrar_cliente === 'sim' && order.tipo_pagamento === 'dinheiro') {
        const valorPedido = parseFloat(order.valor_pedido) || 0;
        if (valorPedido > 0) {
          await pool.query(
            "UPDATE users SET credit = credit + $1 WHERE username=$2",
            [valorPedido, order.loja_user]
          );
        }
      }
    }
